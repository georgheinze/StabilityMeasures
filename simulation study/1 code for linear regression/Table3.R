###############################################################################
##############  Table 3: Model selection frequencies ########################
###### Supplementary Material S5: MSF for BE(AIC), BE(0.05), Lasso ###########
###############################################################################
library(xlsx)
...
sub_bw5_mat50 <- corr.mod(results_sub50, "bw5_betas")[1:3, ]
sub_bw5_mat63 <- corr.mod(results_sub63, "bw5_betas")[1:3, ]
sub_bw5_mat80 <- corr.mod(results_sub80, "bw5_betas")[1:3, ]
boot_bw5_mat <- corr.mod(results_boot, "bw5_betas")[1:3, ]


sub_la_mat50 <- corr.mod(results_sub50, "la_betas")[1:3, ]
sub_la_mat63 <- corr.mod(results_sub63, "la_betas")[1:3, ]
sub_la_mat80 <- corr.mod(results_sub80, "la_betas")[1:3, ]
boot_la_mat <- corr.mod(results_boot, "la_betas")[1:3, ]
...
tab_msf <- function(boot, sub50, sub63, sub80) {
  ...
  colnames(msf) <- paste0("N=", c(150, 300, 750))
  return(msf)
