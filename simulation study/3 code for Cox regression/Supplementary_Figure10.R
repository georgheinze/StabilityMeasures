###############################################################################
##############  Supplementary Figure 10: RCB & RMSDR for the bootstrap #########
###############################################################################
library(car)

load("../1 code for linear regression/setup.RData")
...
calc.dif <- function(object, method, measure, true) {
  ...
  rownames(median_m) <-
    rownames(mad) <- rownames(md) <- colnames(setup[, 4:ncol(setup)])
...
effect <- setup[1, 4:ncol(setup)] != 0


lwds_cat <- cut(R2[names(effect)] * 100, c(0.00001, 0.5, 2, 3, 6))
...
plot.res <-
  function(object = boot,
    ...
    axis(
      1,
      at = 1:ncol(item),
      labels = c("300", "750", "1000", "5000", "10000"),
      cex.axis = 1.8
    )
    if (!is.null(line_at))
      abline(h = line_at, col = "grey65")
    
    for (i in 2:nrow(item)) {
      lines(
        item[i, 1:ncol(item)],
        col = cols[i],
...
# produce a pdf for Supplementary Figure 8




pdf("rcb_rmsdratio.pdf", height = 12.5, width = 12.5)
...
plot.res(
  ...
  xlab = "",
  main = "",
  ylab = "Median of estimated RCB (x100)"
)
...
plot.res(
  ...
  xlab = "",
  main = "",
  ylab = "Median of estimated RMSD ratio"
)
