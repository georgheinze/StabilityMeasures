###############################################################################################
##############  Supplementary Figure 9: VIF for S0.5, S0.632, S0.8 and the bootstrap ##########
###############################################################################################
...
bwAIC_sub50 <-
  sub_bwAIC_mat50[seq(1, nrow(sub_bwAIC_mat50) - 1, 2),][1:3,]
bwAIC_sub63 <-
  sub_bwAIC_mat63[seq(1, nrow(sub_bwAIC_mat63) - 1, 2),][1:3,]
bwAIC_sub80 <-
  sub_bwAIC_mat80[seq(1, nrow(sub_bwAIC_mat80) - 1, 2),][1:3,]
bwAIC_boot <-
  boot_bwAIC_mat[seq(1, nrow(boot_bwAIC_mat) - 1, 2),][1:3,]
...
rownames(bwAIC_tr) <-
  ...
  rownames(bwAIC_boot) <- c("N = 300", "N = 750", "N = 1000")
...
sub_bwAIC_mat50 <- rmse.beta(results_sub50, "bwAIC_betas")
sub_bwAIC_mat63 <- rmse.beta(results_sub63, "bwAIC_betas")
sub_bwAIC_mat80 <- rmse.beta(results_sub80, "bwAIC_betas")
boot_bwAIC_mat <- rmse.beta(results_boot, "bwAIC_betas")
bench_bwAIC_mat <- rmse.bench(results_sub50, "bwAIC_betas")
...
plot_varsel <- function(mat_melt, n = c(300, 750, 1000)) {
  # rank by R2
...
plot_rmse <- function(mat_melt, n = c(300, 750, 1000)) {
  # rank by R2
  mat_melt$Variable <- factor(
    mat_melt$Variable,
    levels =
